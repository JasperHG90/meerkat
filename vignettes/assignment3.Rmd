...
The function `emp_power()` calculates, for a given $R$ number of repititions, the t-statistic between the mean value of the control group and that of a random sample drawn from the normal distribution. Given that we are interested in the probability $\pi(\theta)$, a reasonable approach is to take the proportion of cases where the null hypothesis is rejected. 
...
```{r}
power.t.test(n=50, delta=5, sd=15, type="one.sample", 
...
```{r}
emp <- emp_power(ref_mu=150, n=50, mu=155, sd=15, alternative="greater", R=1000)
emp$power
```
